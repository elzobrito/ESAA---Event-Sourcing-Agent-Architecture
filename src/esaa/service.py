...
class ESAAService:
    ...
    def clear_activity(
        ...
        roadmap, issues, lessons = materialize([])
        save_roadmap(self.root, roadmap)
        save_issues(self.root, issues)
        save_lessons(self.root, lessons)
        verify = self.verify()
    ...
    def report_issue(
        ...
        task = self.task_state(task_id)["task"]
        prior_status = task["status"] if task["status"] != "done" else "review"
        activity_event: dict[str, Any] = {
            "action": "issue.report",
            "task_id": task_id,
        ...
        try:
            ...
            if file_updates:
                write_event = make_event(
                    ...
                    payload={"task_id": task_id, "files": [normalize_rel_path(item["path"]) for item in file_updates]}
                )
    ...
    def _accept_agent_output(
        ...
        if file_updates:
            ...
            write_event = make_event(
                ...
                payload={"task_id": task["task_id"], "files": [normalize_rel_path(item["path"]) for item in file_updates]},
            )
            candidate_events.append(write_event)
            _ = materialize(events + new_events + candidate_events)
            ...
            if wave_write_set is not None:
                wave_write_set.extend(current_write_set)
    ...
    def run(self, steps: int | None = 1, dry_run: bool = False, parallel: int = 1) -> dict[str, Any]:
        ...
        while steps is None or iteration < steps:
            ...
            candidates = [t for t in roadmap["tasks"]
            ...
            contexts = [(task, build_dispatch_context(roadmap, task, contract, schema=schema)) for task in runnable_wave]
...
def load_plugin_seeds(root: Path) -> dict[str, Any] | None:
    """R9 — Loader generico de plugins roadmap.*.json.
