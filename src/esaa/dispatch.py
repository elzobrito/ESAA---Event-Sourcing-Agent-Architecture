...
_FIELDS_BY_ACTION = {
    ...
    "issue.report": {
        "action", "task_id", "prior_status",
        "issue_id", "severity", "title", "evidence",
        "category", "subtype", "affected", "fixes", "lesson",
    },
}
...
def slice_schema(schema: dict[str, Any], allowed: Iterable[str]) -> dict[str, Any]:
    ...
    ev["allOf"] = [
        branch for branch in ev.get("allOf", [])
...
def filter_lessons(lessons: list[dict[str, Any]], task_kind: str, expected_action: str) -> list[dict[str, Any]]:
...
def filter_issues(issues: list[dict[str, Any]], task: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for issue in issues:
        ...
        link_match = task_id in {links.get("reported_by_task_id"), links.get("fixes_task_id"), links.get("hotfix_task_id")}
...
def build_minimal_context(
    ...
    for optional in ("is_hotfix", "issue_id", "fixes", "scope_patch", "required_verification", "baseline_id"):
        if optional in task:
            ctx["task"][optional] = task[optional]
